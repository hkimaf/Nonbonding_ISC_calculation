program duschinsky_read
    implicit none
    external sgetrf
    integer :: natom = 16, nmodes, arg_count, i, j, k, n, row, nsteps, test_IPIV(2), test_INFO
...
! Insert the time step, in atomic units
    time_step = 0.1
    integrated_result = time_step
! Insert the SOC matrix element
    soc = 0.000140 * c_cm * atomic_time * 2 * 3.141592
    ...
    damping = 5.0 * c_cm * atomic_time * 2 * 3.141592
    ...
    do i= 1, maxlines
        ...
        if (j/=0) exit
        row=counter
        read(line,'(42f25.20)') duschinsky_matrix(row,:)
        counter = counter+1
    end do
    close(unit=10)
    ...
    do i=1,maxlines
        ...
        if (j/=0) exit
        row=counter
        read(line,'(42f25.20)') displacement_vector(row)
        counter=counter+1
    end do
    close(unit=10)
    ...
    do i=1,maxlines
        ...
        if (j/=0) exit
        row=counter
        read(line,'(42f25.20)') s1_freq(row)
!        s1_freq(row)=1e-15/atomic_time*s1_freq(row)
    ...
    do i=1,maxlines
        ...
        if (j/=0) exit
        row=counter
        read(line,'(42f25.20)') t1_freq(row)
!        t1_freq(row)=1e-15/atomic_time*t1_freq(row)
    ...
    do i = 1, nsteps
        time = i * time_step
        call RHO_DSO(nmodes, s1_freq, t1_freq, duschinsky_matrix, displacement_vector, time, temperature, partition, adiabatic, damping, sqrt_previous, new_sqrt, correlation)
...
!############
 ...
 !               write(*,*) "LU decomposition successful."
        ...
        subroutine RHO_DSO(nmodes,S1FREQ,T1FREQ,duschinsky_matrix,displacement_vector,time,temperature,partition,adiabatic,damping,sqrt_previous,sqrt_updated,correlation)
            integer, intent(in) :: nmodes
            integer :: i,j

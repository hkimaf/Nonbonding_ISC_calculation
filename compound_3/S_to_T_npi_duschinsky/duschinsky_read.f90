...
! Insert the time step, in atomic units
    time_step = 0.01
    integrated_result = 0.5 * 1.09767360975232 * time_step
! Insert the SOC matrix element
    soc = SQRT(3.0) * 1.802720 * c_cm * atomic_time * 2 * 3.141592
...
!        t1_freq(row)=1e-15/atomic_time*t1_freq(row)
    ...
    do i = 1, nsteps
        time = i * time_step
        call RHO_DSO(nmodes, t1_freq, s1_freq, duschinsky_matrix, displacement_vector, time, temperature, partition, adiabatic, sqrt_previous, new_sqrt, correlation)
...
!############
 ...
 !               write(*,*) "LU decomposition successful."
        ...
        subroutine RHO_DSO(nmodes,T1FREQ,S1FREQ,duschinsky_matrix,displacement_vector,time,temperature,partition,adiabatic,sqrt_previous,sqrt_updated,correlation)
            integer, intent(in) :: nmodes
            integer :: i,j

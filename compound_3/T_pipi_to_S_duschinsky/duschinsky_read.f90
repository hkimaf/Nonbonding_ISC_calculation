program duschinsky_read
    implicit none
    external sgetrf
    integer :: natom = 22, nmodes, arg_count, i, j, k, n, row, nsteps, test_IPIV(2), test_INFO
...
! Insert the adiabatic energy difference
    adiabatic = 3.775399999312867e-04
...
! Insert the SOC matrix element
    soc = 0.0118 * c_cm * atomic_time * 2 * 3.141592
    ...
    do i= 1, maxlines
        ...
        if (j/=0) exit
        row=counter
        read(line,'(60f25.20)') duschinsky_matrix(row,:)
        counter = counter+1
    end do
    close(unit=10)
    ...
    do i=1,maxlines
        ...
        if (j/=0) exit
        row=counter
        read(line,'(60f25.20)') displacement_vector(row)
        counter=counter+1
    end do
    close(unit=10)
    ...
    do i=1,maxlines
        ...
        if (j/=0) exit
        row=counter
        read(line,'(60f25.20)') s1_freq(row)
!        s1_freq(row)=1e-15/atomic_time*s1_freq(row)
    ...
    do i=1,maxlines
        ...
        if (j/=0) exit
        row=counter
        read(line,'(60f25.20)') t1_freq(row)

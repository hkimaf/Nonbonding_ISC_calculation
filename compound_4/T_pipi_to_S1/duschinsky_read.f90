program duschinsky_read
    implicit none
    external sgetrf
    integer :: natom = 23, nmodes, arg_count, i, j, k, n, row, nsteps, test_IPIV(2), test_INFO
...
! Insert the SOC matrix element
    soc = 0.014216893955256 * c_cm * atomic_time * 2 * 3.141592
    ...
    do i= 1, maxlines
        ...
        if (j/=0) exit
        row=counter
        read(line,'(63f25.20)') duschinsky_matrix(row,:)
        counter = counter+1
    end do
    close(unit=10)
    ...
    do i=1,maxlines
        ...
        if (j/=0) exit
        row=counter
        read(line,'(63f25.20)') displacement_vector(row)
        counter=counter+1
    end do
    close(unit=10)
    ...
    do i=1,maxlines
        ...
        if (j/=0) exit
        row=counter
        read(line,'(63f25.20)') s1_freq(row)
!        s1_freq(row)=1e-15/atomic_time*s1_freq(row)
    ...
    do i=1,maxlines
        ...
        if (j/=0) exit
        row=counter
        read(line,'(63f25.20)') t1_freq(row)
!        t1_freq(row)=1e-15/atomic_time*t1_freq(row)
    ...
    do i=1,nmodes
        partition = partition * EXP(-0.5 * t1_freq(i) / (boltzmann * temperature)) / (1 - EXP(-t1_freq(i) / (boltzmann * temperature)))
    end do
    write(*,*) "The partition is", partition

    test_partition = EXP(-0.5 * test_t1_freq(1) / (boltzmann * temperature)) / (1 - EXP(-test_t1_freq(1) / (boltzmann * temperature)))
    filename = 'correlation.txt'
    open(unit = 10, file = filename, status = 'replace', action = 'write')
    do i = 1, nsteps
        time = i * time_step
        call RHO_DSO(nmodes, s1_freq, t1_freq, duschinsky_matrix, displacement_vector, time, temperature, partition, adiabatic, sqrt_previous, new_sqrt, correlation)
...
!############
    ...
    contains
        subroutine VECTORS1(nmodes,S1FREQ,time,sin_matrix,tan_matrix)
            ...
            do i=1,nmodes
                sin_matrix(i,i)=S1FREQ(i)/SIN(S1FREQ(i)*time)
                tan_matrix(i,i)=S1FREQ(i)/TAN(S1FREQ(i)*time)
            end do
        end subroutine VECTORS1

        subroutine VECTORT1(nmodes,T1FREQ,time,temperature,sin_matrix,tan_matrix)
            integer :: i
            integer, intent(in) :: nmodes
            ...
            double complex, dimension(nmodes,nmodes),intent(out) :: sin_matrix, tan_matrix
            do i=1,nmodes
                sin_matrix(i,i)=T1FREQ(i)/SIN(T1FREQ(i)*CMPLX(-time,-1/(boltzmann*(unit_time/atomic_time)*temperature)))
                tan_matrix(i,i)=T1FREQ(i)/TAN(T1FREQ(i)*CMPLX(-time,-1/(boltzmann*(unit_time/atomic_time)*temperature)))
            end do
        end subroutine VECTORT1

        subroutine MATRIX_CREATION(nmodes,S1FREQ,T1FREQ,duschinsky_matrix,displacement_vector,time,temperature,s_sin_matrix,s_tan_matrix,t_sin_matrix,t_tan_matrix,E_matrix,A_matrix,B_matrix,K_matrix,f_vector)
            integer :: i,j,k
            integer, intent(in) :: nmodes
            ...
            call VECTORS1(nmodes,S1FREQ,time,s_sin_matrix,s_tan_matrix)
            call VECTORT1(nmodes,T1FREQ,time,temperature,t_sin_matrix,t_tan_matrix)
            do i = 1, nmodes
                E_matrix(i,i) = t_tan_matrix(i,i) - t_sin_matrix(i,i)
                A_matrix(i,i) = s_sin_matrix(i,i)
                B_matrix(i,i) = s_tan_matrix(i,i)
                do j=1,nmodes
                    do k=1,nmodes
                        A_matrix(i,j) = A_matrix(i,j) + duschinsky_matrix(k,i) * t_sin_matrix(k,k) * duschinsky_matrix(k,j)
                        B_matrix(i,j) = B_matrix(i,j) + duschinsky_matrix(k,i) * t_tan_matrix(k,k) * duschinsky_matrix(k,j)
                    end do
 ...
 !               write(*,*) "LU decomposition successful."
        ...
        subroutine RHO_DSO(nmodes,S1FREQ,T1FREQ,duschinsky_matrix,displacement_vector,time,temperature,partition,adiabatic,sqrt_previous,sqrt_updated,correlation)
            integer, intent(in) :: nmodes
            integer :: i,j
            real(8) :: sqrt_difference_abs, sqrt_sum_abs
            ...
            call MATRIX_CREATION(nmodes,S1FREQ,T1FREQ,duschinsky_matrix,displacement_vector,time,temperature,s_sin_matrix,s_tan_matrix,t_sin_matrix,t_tan_matrix,E_matrix,A_matrix,B_matrix,K_matrix,f_vector)
            ...
            new_sqrt = SQRT(prod_sin_sin / partition / k_determinant * prod_tri_sin / partition)
            ...
            correlation = EXP(CMPLX(0, -1) * adiabatic * time) * sqrt_updated * EXP(CMPLX(0, -1) * (0.5 * fKf - dEd))
